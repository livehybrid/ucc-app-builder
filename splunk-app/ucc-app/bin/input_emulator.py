...
class _StubInputs:
    """Stand-in for splunklib.modularinput's InputDefinition for a Script subclass:
    `inputs.inputs` is {stanza: {param: value}}. We expose one emulated stanza built from
    the user-supplied args so `for name, item in inputs.inputs.items()` yields it."""

    def __init__(self, args, index):
        item = dict(args or {})
        item.setdefault('index', index)
        self.inputs = {'emulated://%s' % (args.get('name') or 'input'): item}
        self.metadata = {'server_host': 'localhost', 'server_uri': 'https://127.0.0.1:8089',
                         'session_key': '', 'checkpoint_dir': '/tmp'}
...
def main():
    ...
    try:
        ...
        if callable(fn):
            ...
            # No module-level collect_events/stream_events - try a splunklib.modularinput
            # Script SUBCLASS, whose collection lives in stream_events(self, inputs, ew).
            import inspect as _inspect
            cls = next((v for v in ns.values()
                        if _inspect.isclass(v) and v.__name__ != 'Script'
                        and callable(getattr(v, 'stream_events', None))), None)
            if cls is None:
                print(json.dumps({'ok': False,
                                  'error': 'no collect_events/stream_events function or Script '
                                           'subclass with stream_events was found',
                                  'logs': logs}))
                return
            try:
                inst = cls()
            except Exception:
                inst = cls.__new__(cls)  # skip a custom __init__ that needs real Splunk wiring
            # Script.stream_events(self, inputs, ew): iterate inputs.inputs.items(). Stub a
            # single configured input stanza from the user-supplied args.
            inst.stream_events(_StubInputs(args, index), ew)
