...
export function ImportExport({ onImportComplete }: ImportExportProps) {
  ...
  const generatedFiles = analysis?.files.filter((f) => f.origin === 'generated' || f.origin === 'modified-generated') || [];
    ...
    <List>
      {files.map((f) => (
        <List.Item key={f.path}>
          <span style={{ fontFamily: 'Splunk Platform Mono, Inconsolata, Consolas, monospace', fontSize: '0.85rem' }}>
            {f.path}
          </span>
        </List.Item>
      ))}
      ...
      {analysis && (
        <AnalysisResults>
          ...
          {analysis.warnings.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              {analysis.warnings.map((warning, i) => (
                <Message key={i} type="warning">{warning}</Message>
              ))}
            </div>
          )}
          ...
          <div style={{ marginTop: 24 }}>
            <Button appearance="primary" onClick={() => onImportComplete(analysis)} label="Import to Editor" />
          </div>
        </AnalysisResults>
      )}
    </ImportContainer>
