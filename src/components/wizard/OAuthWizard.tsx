...
const StepDot = styled.div<{ $active: boolean; $completed: boolean }>`
    ...
    background-color: ${props => 
        props.$active ? variables.brandColor : 
        props.$completed ? variables.successColor : 
        '#ccc'};
`;
...
export function OAuthWizard({ open, onClose, onSave, initialConfig }: OAuthWizardProps) {
    ...
    const isValidStep2 = config.authType === 'oauth2' 
                ...
                <WizardContainer>
                        ...
                        <>
                            ...
                            <FormRow>
                                ...
                                <Select 
                                    ...
                                    onChange={(_e, { value }) => setConfig({...config, authType: value as OAuthConfiguration['authType']})}
                                >
                            ...
                            <Message type="info">
                                {config.authType === 'oauth2' && "Best for modern REST APIs. Handles token refresh automatically."}
                    ...
                    {step === 2 && config.authType === 'oauth2' && (
                        <>
                            ...
                            <FormRow>
                                ...
                                <Text value={config.clientId || 'client_id'} onChange={(_e, { value }) => setConfig({...config, clientId: value})} />
                            </FormRow>
                            <FormRow>
                                <label>Client Secret Field Name</label>
                                <Text value={config.clientSecret || 'client_secret'} onChange={(_e, { value }) => setConfig({...config, clientSecret: value})} />
                            </FormRow>
                            <FormRow>
                                <label>Authorization URL</label>
                                <Text value={config.authUrl} onChange={(_e, { value }) => setConfig({...config, authUrl: value})} placeholder="https://api.example.com/oauth/authorize" />
                            </FormRow>
                            <FormRow>
                                <label>Token URL</label>
                                <Text value={config.tokenUrl} onChange={(_e, { value }) => setConfig({...config, tokenUrl: value})} placeholder="https://api.example.com/oauth/token" />
                            </FormRow>
                            <FormRow>
                                <label>Redirect URI (Callback)</label>
                                <Text value={config.redirectUri} onChange={(_e, { value }) => setConfig({...config, redirectUri: value})} />
                            </FormRow>
                        </>
                    )}
                    ...
                    {step === 3 && (
                        <>
                            ...
                            <Text>This will add fields to your Global Config and create necessary handler scripts.</Text>
                        </>
                    )}
                </WizardContainer>
            </Modal.Body>
