  ...
  switch (validator.type as ValidatorType) {
    case 'string': {
      if (validator.minLength !== undefined && value.length < validator.minLength) {
        return { valid: false, error: defaultError(`Minimum length is ${validator.minLength} characters`) };
      }
      if (validator.maxLength !== undefined && value.length > validator.maxLength) {
        return { valid: false, error: defaultError(`Maximum length is ${validator.maxLength} characters`) };
      }
      return { valid: true };
    }
 ...
 */
export function validateRequired(value: string | undefined | null, required: boolean): ValidationResult {
