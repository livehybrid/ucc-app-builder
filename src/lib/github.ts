...
export async function initiateDeviceFlow(clientId: string): Promise<DeviceFlowResponse> {
  // Use Vite proxy to bypass CORS (proxies to https://github.com/login/device/code)
  const response = await fetch('/github-api/login/device/code', {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    ...
    console.error('Device flow error:', text);
...
export async function pollForToken(clientId: string, deviceCode: string, interval: number): Promise<GitHubAuth> {
  const check = async (): Promise<GitHubAuth | null> => {
    // Use Vite proxy to bypass CORS (proxies to https://github.com/login/oauth/access_token)
    const response = await fetch('/github-api/login/oauth/access_token', {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
...
export async function createRepo(token: string, name: string, isPrivate: boolean, description?: string): Promise<GitHubRepo> {
 ...
 */
async function getRepoTree(token: string, owner: string, repo: string, branch: string = 'main'): Promise<GitTreeItem[]> {
  try {
 ...
 */
async function getFileContent(token: string, owner: string, repo: string, path: string, ref: string = 'main'): Promise<string> {
...
export async function cloneRepo(
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      const binaryExts = ['png', 'jpg', 'jpeg', 'gif', 'ico', 'woff', 'woff2', 'ttf', 'eot', 'zip', 'tar', 'gz', 'exe', 'dll', 'so', 'pyc'];
      ...
      errors.push(`Failed to fetch ${file.path}: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
...
async function getRef(token: string, owner: string, repo: string, ref: string = 'heads/main'): Promise<GitRef> {
  try {
    return await request<GitRef>(`/repos/${owner}/${repo}/git/ref/${ref}`, {}, token);
  } catch (e: unknown) {
    if (e instanceof Error && 'status' in e && (e as { status: number }).status === 404 && ref === 'heads/main') {
...
async function createTree(token: string, owner: string, repo: string, baseTreeSha: string | undefined, tree: Record<string, unknown>[]): Promise<string> {
  const body: Record<string, unknown> = { tree };
  if (baseTreeSha) body.base_tree = baseTreeSha;

  const response = await request<{ sha: string }>(`/repos/${owner}/${repo}/git/trees`, {
    method: 'POST',
...
async function createCommit(token: string, owner: string, repo: string, message: string, treeSha: string, parentSha?: string): Promise<string> {
  const body: Record<string, unknown> = {
    message,
    tree: treeSha,
    parents: parentSha ? [parentSha] : [],
  };
  const response = await request<{ sha: string }>(`/repos/${owner}/${repo}/git/commits`, {
    method: 'POST',
    body: JSON.stringify(body),
  }, token);
  return response.sha;
...
async function updateRef(token: string, owner: string, repo: string, ref: string, sha: string): Promise<void> {
  await request(`/repos/${owner}/${repo}/git/refs/${ref}`, {
    method: 'PATCH',
    body: JSON.stringify({
      sha,
...
export async function pushFiles(
  ...
  try {
    const ref = await getRef(token, owner, repoName, 'heads/main').catch(() => getRef(token, owner, repoName, 'heads/master'));
    refName = ref.ref.replace('refs/', '');
    parentSha = ref.object.sha;
    
    // Get commit to get tree
    const commit = await request<GitCommit>(`/repos/${owner}/${repoName}/git/commits/${parentSha}`, {}, token);
  ...
  if (!parentSha) {
     ...
     // We have commit now.
     // Try creating ref.
     await request(`/repos/${owner}/${repoName}/git/refs`, {
         method: 'POST',
