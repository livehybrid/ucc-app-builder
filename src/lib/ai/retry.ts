...
export class RetryError extends Error {
    constructor(message: string, public lastError: unknown, public status?: number) {
        super(message);
        ...
        try {
            ...
            if (opts.retryableStatus.includes(response.status)) {
                if (attempt < opts.maxRetries) {
                    ...
                    console.log(`Request failed with ${response.status}. Retrying in ${waitTime}ms (Attempt ${attempt + 1}/${opts.maxRetries})`);
