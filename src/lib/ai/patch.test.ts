...
const makeReader = (files: Record<string, string>) => (p: string) =>
...
describe('parsePatch', () => {
  it('parses a simple update hunk', () => {
    ...
    expect(parsed.files[0].hunks[0].contextAndMinus).toEqual([
      'def hello():',
      '    return 1',
    ]);
    expect(parsed.files[0].hunks[0].contextAndPlus).toEqual([
      'def hello():',
      '    return 2',
    ]);
  });
