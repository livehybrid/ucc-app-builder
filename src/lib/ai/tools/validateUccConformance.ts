...
export const validateUccConformance: Tool = {
  ...
  execute: async (_args, vfs) => {
    ...
    const globalConfigPath = files.find((p) => p.endsWith('/globalConfig.json')) || files.find((p) => p === 'globalConfig.json');
    if (!globalConfigPath) {
    ...
    if (!services.length) {
      issues.push({ severity: 'warning', message: 'No modular input services defined in globalConfig.pages.inputs.services.' });
    }
    ...
    for (const service of services) {
      ...
      if (!scriptPath) {
        issues.push({ severity: 'error', message: `Missing main modular input script for "${inputName}" at package/bin/${inputName}.py.` });
      } else {
      ...
      if (!helperPath) {
        issues.push({ severity: 'warning', message: `Missing helper module for "${inputName}" at package/bin/${inputName}_helper.py.` });
      } else {
