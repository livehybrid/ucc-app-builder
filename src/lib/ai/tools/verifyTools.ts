...
async function fetchCapabilities(): Promise<{ dockerToolsEnabled: boolean; browserCheckEnabled: boolean }> {
  try {
...
export const runUccGen: Tool = {
  ...
  execute: async (args, vfs) => {
    ...
    const result = await postJson<{
      ok: boolean; tarball?: string; logs?: string[]; error?: string;
    }>('/api/agent/ucc-gen', {
    ...
    if (!result.ok) return `ucc-gen failed: ${result.error}\n${(result.logs ?? []).slice(-20).join('\n')}`;
...
export const installToSplunkDocker: Tool = {
  ...
  parameters: {
    ...
    properties: {
      ...
      containerName: { type: 'string', description: 'Container name. Default ucc-app-builder-splunk.' },
    },
  },
  execute: async (args) => {
    ...
    if (!result.ok && result.error) return `Install failed: ${result.error}\n${(result.logs ?? []).slice(-20).join('\n')}`;
    const errCount = result.errors?.length ?? 0;
    const head = errCount
      ? `Installed with ${errCount} splunkd.log errors.`
      : `Installed cleanly.`;
