...
function createBaseVfs(): VirtualFileSystem {
  ...
  vfs.writeFile('/demo/globalConfig.json', JSON.stringify({
    pages: {
      inputs: {
...
describe('validateUccConformance tool', () => {
  it('returns pass when required UCC files exist', async () => {
    ...
    vfs.writeFile('/demo/package/bin/demo_input.py', 'import import_declare_test\nfrom splunktaucclib.modinput_wrapper import base_modinput\n');
    vfs.writeFile('/demo/package/bin/demo_input_helper.py', 'def collect_events(helper, ew):\n    return\n');
