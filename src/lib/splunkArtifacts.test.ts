...
describe('buildDashboardDefinition', () => {
    ...
    panels: [
      {
        ...
        spl: 'index=web status>=400 | timechart count',
        viz: 'line' as const,
      },
      {
        title: 'By endpoint',
        spl: 'index=web status>=400 | top uri',
        viz: 'bar' as const,
        earliest: '-7d',
      },
    ],
  ...
  it('emits Studio dataSources + visualizations + grid layout for each panel', () => {
    ...
    const viz = d.visualizations as Record<
      string,
      { type: string; dataSources: { primary: string } }
    >;
    ...
    const layout = d.layout as {
      type: string;
      structure: Array<{ item: string; position: { x: number; w: number } }>;
    };
  ...
  it('honours a per-panel time range', () => {
    const d = buildDashboardDefinition(spec);
    const ds = d.dataSources as Record<
      string,
      { options: { queryParameters: { earliest: string } } }
    >;
...
describe('buildDashboardViewXml', () => {
  it('wraps the definition in a version=2 dashboard with valid embedded JSON', () => {
    const xml = buildDashboardViewXml({
      title: 'My & Board',
      panels: [{ title: 't', spl: 'index=x', viz: 'single' }],
    });
