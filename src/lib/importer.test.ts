...
import { importAppFromZip, classifyFileOrigin, createManifestFromImport, extractSourceFiles } from './importer';
...
label = Test Add-on`,
  ...
  it('should handle malformed JSON in globalConfig gracefully', async () => {
    ...
    expect(analysis.warnings.some(w => w.includes('Could not parse globalConfig.json'))).toBe(true);
