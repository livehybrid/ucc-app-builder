...
describe('applyTemplate', () => {
    ...
    const template = makeTemplate({
      accounts: [{
        config: {
          ...
          fields: [
            { field: 'api_key', label: 'API Key', type: 'password' as const, required: true, encrypted: true },
          ],
        },
      }],
    });
  ...
  it('should skip duplicate accounts', () => {
    ...
    expect(result.summary.some(s => s.includes('Skipped') || s.includes('already exists'))).toBe(true);
  });
